...
import com.meicm.cas.screenhealth.AppState.isGatheringCategories
import com.meicm.cas.screenhealth.AppState.isGatheringUnlocks
import com.meicm.cas.screenhealth.AppState.isGatheringUsageStats
    ...
    private fun processAppsUsageSessions(manager: UsageStatsManager, startTime: Long, endTime: Long)
        ...
        currentAppTimestamps.forEach {
            if (it.value != null) {
                ...
                Log.d(
                    Const.LOG_TAG,
                    "APP: ${com.meicm.cas.screenhealth.util.getAppName(it.key)} used for: ${hms.first}h ${hms.second}min ${hms.third}s FROM ${epochToString(it.value!!)} TO ${epochToString(Calendar.getInstance().timeInMillis)}"
                )*/
            }
        }
